...
```https://github.com/shiryu2002/clawd-pet.git をcloneしてreadmeを読んでセットアップして```
...
#### for AI Agent
```
ユーザーのために上記エイリアスを設定するときは、`/絶対パス/` を実際の clone 先（`git rev-parse --show-toplevel` で取得）に置換し、`~/.bashrc` ではなくユーザーが普段使うシェルの rc ファイルへ追記すること。`alias` 行が既にあれば重複追加しない。
```
